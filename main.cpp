...
Vector3 Lerp(const Vector3& v1, const Vector3& v2, float t) {
	...
	resultLerp.x = t * v1.x + (1 - t) * v2.x;
	resultLerp.y = t * v1.y + (1 - t) * v2.y;
	resultLerp.z = t * v1.z + (1 - t) * v2.z;
...
void DrawBezier(const Vector3& controlPoint0, const Vector3& controlPoint1, const Vector3& controlPoint2,
	...
	for (int index = 0; index < 32; ++index) {
		...
		Vector3 left = Lerp(Lerp(controlPoint0, controlPoint1, t0), Lerp(controlPoint1, controlPoint2, t0), t0);
		Vector3 right = Lerp(Lerp(controlPoint0, controlPoint1, t1), Lerp(controlPoint1, controlPoint2, t1), t1);

		left = Transform(left, Multiply(viewProjectionMatrix, viewportMatrix));
		right = Transform(right, Multiply(viewProjectionMatrix, viewportMatrix));

		Novice::DrawLine((int)left.x, (int)left.y, (int)right.x, (int)right.y, color);
	}
}
